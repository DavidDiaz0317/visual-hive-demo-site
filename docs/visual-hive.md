...
- Reasons: Detected Storybook story Visual Hive Test Lab/Fixtures in src/App.stories.tsx; starter screenshots target /iframe.html?id=visual-hive-test-lab-fixtures--command-matrix-row&viewMode=story., The starter contract uses Storybook's framework-owned iframe root so an empty or failed story cannot satisfy a body-only assertion., Desktop and mobile Storybook screenshots give a PR-safe component visual lane without requiring Chromatic.
